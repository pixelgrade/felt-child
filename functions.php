<?php
/**
 * Felt Child functions and definitions
 *
...
/**
 * Setup Felt Child Theme's textdomain.
 *
 * Declare textdomain for this child theme.
 * Translations can be filed in the /languages/ directory.
 */
function felt_child_theme_setup() {
	load_child_theme_textdomain( 'felt-child-theme', get_stylesheet_directory() . '/languages' );
}
add_action( 'after_setup_theme', 'felt_child_theme_setup' );
...
function felt_child_enqueue_styles() {
	...
	if ( !is_rtl() ) {
		wp_enqueue_style( 'felt-style', get_template_directory_uri() . '/style.css', array(), $parent->get( 'Version' ) );
	} else {
		wp_enqueue_style( 'felt-style', get_template_directory_uri() . '/rtl.css', array(), $parent->get( 'Version' ) );
	}
	// Here we are adding the child style.css while still retaining
	// all of the parents assets (style.css, JS files, etc)
	wp_enqueue_style( 'felt-child-style',
		get_stylesheet_directory_uri() . '/style.css',
		array('felt-style') //make sure the the child's style.css comes after the parents so you can overwrite rules
	);

}

add_action( 'wp_enqueue_scripts', 'felt_child_enqueue_styles' );
...
function felt_child_overwrite_files() {
	...
	// See the parent's function.php -> the felt_scripts_styles() function
	// for details like resources names
	//
	// Remember that the rest of the static resources will still get loaded like 
	// felt-imagesloaded, felt-hoverintent and felt-velocity

		wp_dequeue_script( 'felt-scripts' );
	...
	// This is not required, but I assume you are not modifying that much :)
		...
		wp_enqueue_script( 'felt-child-scripts',
			get_stylesheet_directory_uri() . '/assets/js/main.js',
			array( 'jquery', 'masonry', 'felt-imagesloaded', 'felt-hoverintent', 'felt-velocity' ),
			'1.0.0', true );
	...
	// see the parent's function.php -> the felt_scripts_styles() function for details like resources names
	
		wp_dequeue_style( 'felt-style' );
	...
	// Now you can add your own, modified version of the "style.css" file
		...
		wp_enqueue_style( 'felt-child-style',
			get_stylesheet_directory_uri() . '/style.css',
			array('felt-font-awesome-style') //use the same dependencies as the parent because we want to still use FontAwesome icons
		);
}

// Load the files from the function mentioned above:

	add_action( 'wp_enqueue_scripts', 'felt_child_overwrite_files', 11 );
